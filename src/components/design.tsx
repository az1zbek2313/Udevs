...
import mobileImage from "@/../public/mobileIman2.png";
      ...
      <div className={`${styles.container}`}>
        ...
        <div className={`${styles.flex} ww:justify-between w-full `}>
          ...
          <div className="flex flex-col justify-start ss:max-w-[500px] ww:w-[47%]">
            <p className="text-[16px] mt-6 leading-[30px] text-dark font-medium ww:text-[24px] ww:leading-[40px] ">
            {t("designText")}
            </p>
            ...
            <div className="my-6">
              ...
              <div className="flex items-center justify-start gap-[48px]">
                {
                  DesignTechnologiesCards.map((el) => (
                    <div className={`flex flex-col justify-center items-center ${(el.id === 3) && "hidden xs:inline-block"}`} key={el.id}>
                      <Image src={el.img} alt="icon" width={56} height={56} className={`${el.id === 3 && "w-[40px] h-[56px]"} mb-2`}/>
                      <span className="text-dark font-semibold leading-[30px] text-[20px]">{el.title}</span>
                    </div>
                  ))
                }
              </div>
